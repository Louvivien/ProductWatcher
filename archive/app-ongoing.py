...
def get_proxies(url):
    ...
    return [(proxy.split(':')[0], proxy.split(':')[1]) for proxy in proxies]

def find_working_proxy(proxies):
    url = "http://httpbin.org/ip"  # A simple URL to check the IP address
    for proxy in proxies:
        formatted_proxy = f"http://{proxy[0]}:{proxy[1]}"  # Format the proxy in the correct format
        try:
            response = requests.get(url, proxies={"http": formatted_proxy, "https": formatted_proxy}, timeout=60)
            if response.status_code == 200:
                return proxy
        except:
            pass  # if the request fails for any reason, just move on to the next proxy
    return None
...
print("working_proxy : ", working_proxy)
...
def switch_proxy():
    ...
    for proxy in proxies:
        if proxy != working_proxy and find_working_proxy(proxies):
            working_proxy = proxy
            break
...
def search_stockx(product_name):
    ...
    # Set up the proxy
    proxies = {
        'https': f"http://{working_proxy[0]}:{working_proxy[1]}",
    }
