...
async def scrape_product(product_name: str) -> dict:
    ...
    url = 'https://stockx.com/fr-fr/search?s=' + product_name
    try:
...
async def product_detail(request: Request, product_name: str):
    ...
    return templates.TemplateResponse("home.html", {"request": request, "edges": result if result else [], "debug_info": None})
